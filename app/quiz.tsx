...
const Quiz = () => {
  ...
  const handleFinishQuizRef = useRef<(() => Promise<void>) | undefined>(
    undefined,
  );
  ...
  const handleFinish = useCallback(async () => {
    ...
    try {
      const result = await quizState.submitQuizAnswers(
        user?.id as string,
        chatId,
        chatTitle,
      );
  ...
  const renderContent = () => {
    ...
    if (quizState.error && quizState.error !== "quiz_limit") {
      return (
        <QuizErrorScreen
          ...
          theme={theme}
          onRetry={() => quizState.generateQuiz(chatId, user?.id as string)}
    ...
    if (!quizState.questions.length) {
      return (
        <View style={styles.container}>
          <Text style={[styles.text, theme === "dark" && { color: "#B3B3B3" }]}>
    ...
    return q ? (
      <QuizQuestionsScreen
        ...
        isSubmitting={false}
        theme={theme}
        onSelectOption={quizState.selectAnswer}
        onPrevious={() =>
          quizState.goToQuestion(quizState.currentQuestionIndex - 1)
        }
        onNext={() =>
          quizState.goToQuestion(quizState.currentQuestionIndex + 1)
        }
    ...
    >
      ...
      >
        ...
        <Text style={[styles.header, theme === "dark" && { color: "#E0E0E0" }]}>
          {quizState.quizCompleted ? "Quiz Results" : "Ongoing quiz"}
        </Text>
      </View>
