...
export default function PublicQuizDetailScreen() {
    ...
    const updateUserPointsFromQuiz = useUserStore((s) => s.updateUserPointsFromQuiz);
    const {
        fetchQuiz,
        joinQuiz,
        submitQuiz,
    ...
    const handleFinish = useCallback(async () => {
        if (!quizId || !userId || !questions.length || isSubmitting || quizCompleted) return;
        const participationId = usePublicQuizStore.getState().participationByQuiz[quizId];
        ...
        try {
            ...
            response.score > 2 ? Presets.alarm() : Presets.ascent();
        } catch {
            setSubmitError(usePublicQuizStore.getState().quizError ?? 'Could not submit quiz');
        } finally {
    ...
    const renderBody = () => {
        if (!userId) {
            return (
                <View style={styles.centerBlock}>
                    <Text style={[styles.fallbackText, theme === 'dark' && { color: '#B3B3B3' }]}>
        ...
        if (!quizCompleted && !quizError && !isSubmitting && (!detail || loading || !sessionReady)) {
